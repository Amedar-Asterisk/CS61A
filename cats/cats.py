...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    if k > len(paragraphs) or k < 0:
        return ""
    else:
        selected_paragraph = [
            paragraph for paragraph in paragraphs if select(paragraph)
        ]
        return selected_paragraph[k] if k < len(selected_paragraph) else ""
    # END PROBLEM 1
...
def about(subject):
    ...
    #     precessed_paragraph = remove_punctuation(lower(paragraph))
    #     splited_paragraph = split(precessed_paragraph)
    #     return any(word in subject for word in splited_paragraph)

    # return about_subject

    ### shorter one
    return lambda paragraph: any(
        word in subject for word in split(remove_punctuation(lower(paragraph)))
    )
    # END PROBLEM 2
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if (typed, source) in cache and limit <= cache[(typed, source)][1]:
            return (
                cache[(typed, source)][0]
                if limit > cache[(typed, source)][0]
                else limit + 1
            )
        else:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        min_idx = min(
            range(len(word_list)),
            key=lambda k: diff_function(typed_word, word_list[k], limit),
        )
        return (
            word_list[min_idx]
            if diff_function(typed_word, word_list[min_idx], limit) <= limit
            else typed_word
        )
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if len(typed) <= 0 or len(source) <= 0:
        ...
        return 0
    elif limit < max(0, abs(len(typed) - len(source))):
        return limit + 1
    else:
        return furry_fixes(typed[1:], source[1:], limit - (typed[0] != source[0])) + (
            typed[0] != source[0]
        )
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < max(
        0, abs(len(source) - len(typed))
    ):  # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 1
        # END
    elif limit == 0 and typed != source:
        return 1
    elif source in typed or typed in source:
        return abs(len(typed) - len(source))
    elif source[1:] in typed or typed[1:] in source:
        return abs(len(typed) - len(source)) + 1
    elif source[:-1] in typed or typed[:-1] in source:
        return abs(len(typed) - len(source)) + 1
    elif min(len(typed), len(source)) < 2:
        return abs(len(typed) - len(source)) + (typed[0] != source[0])
...
def report_progress(typed, source, user_id, upload):
    ...
    for k in range(len(typed)):
        if typed[k] != source[k]:
            progress["progress"] = k / len(source)
            upload(progress)
            return progress["progress"]
    progress["progress"] = len(typed) / len(source)
    upload(progress)
    return progress["progress"]
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [
        [times_list[k] - times_list[k - 1] for k in range(1, len(times_list))]
        for times_list in tpp
...
def fastest_words(words_and_times):
    ...
    return [
        [
            ...
            if player
            == min(player_indices, key=lambda k: get_time(times, k, word_indice))
        ]

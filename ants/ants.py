...
class Place:
    ...
    def add_insect(self, insect):
        ...
        that it can be overridden in subclasses.2
        """
        insect.add_to(self)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place and not place.is_hive:
            if self.lower_bound <= k <= self.upper_bound:
                if place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        damage = amount
        damage += self.damage if amount >= self.health else 0
...
# The WallAnt class
class WallAnt(Ant):
    """A wallant has large health so can protect home well."""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """A Hungryant can eat a bee at its place entirely at once but has cooldown"""
    ...
    def action(self, gamestate):
        if self.cooldown:
            ...
            if self.place.bees:
                luckcy_bee = random_bee(self.place.bees)
                luckcy_bee.reduce_health(luckcy_bee.health)
...
class TankAnt(ContainerAnt):
    """TankAnt not only provides protection but also make damages to all bees in its palce"""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is waterproof thrower"""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        ThrowerAnt.action(self, gamestate=gamestate)

...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    num_player = player_score % 10
    num_opponent = (opponent_score // 10) % 10
    result = 3 * abs(num_player - num_opponent)
    return max(result, 1)
    # END PROBLEM 2
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    num_score_factors = num_factors(score)
    if num_score_factors == 3 or num_score_factors == 4:
        score += 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_avg_score = 0
    max_index = 1
    avg_score_func = make_averaged(roll_dice, times_called)
    for k in range(1, 11):
        avg_score = avg_score_func(k, dice)

    ...
    def report(self):
        ...
        if self.changed():
            ...
            if self.before < self.after:
                ...
                head = "decreased "
            msg = head + "->".join([str(self.before), str(self.after)])
...
class Account:
    ...
    def deposit(self, amount):
        ...
        self.transactions.append(Transaction(self._tra_id, before, self.balance))
        self._tra_id += 1
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            after = self.balance
            self.transactions.append(Transaction(self._tra_id, before, after))
            self._tra_id += 1
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        changes = make_change(amount=coin, coins=self.coins)
        if changes:
            for change in changes:
                self.coins[change] -= 1
                if self.coins[change] == 0:
                    self.coins.pop(change)
